...
const clubColor = {
  ...
  'City': 'rgb(0, 0, 153)',
  'Cantabs': 'rgb(0, 0, 90)',
  'Champs': 'rgb(255, 153, 0)',
  'Rob Roy': 'rgb(153, 0, 0)',
  'X-Press': 'rgb(0, 0, 0)',
  'Chesterton': 'rgb(249, 222, 6)',
  'St Neots': 'rgb(139, 176, 222)',
  'St Radegund': 'rgb(255, 0, 0)',
};
...
export function read_flat(data) {
  data = d3_dsv.csvParse(data);
  const year = _.uniqBy(data.map(d => d.Year));
  const gender = _.uniqBy(data.map(d => d.Sex));
  ...
  for (let yearNum = 0; yearNum < year.length; yearNum++) {
    for (let genderNum = 0; genderNum < gender.length; genderNum++) {
      ...
      event.days = _.uniqBy(crewsAllDays.map(c => c.Day)).length;

      const numDivisions = _.uniqBy(crewsFirstDay.map(c => c.Division)).length;
