...
function calculateCorrelation() {
  ...
  var colleges = crews
    .filter(function(crew) {
      return !/[0-9]+$/.test(crew.name);
    })
    .map(function(crew) {
      return crew.name;
    });

  var collegeBoats = colleges
  ...
  var correlations = collegeBoats.map(function(boats) {
    ...
    for (var i = 0; i < boats[0].values.length - 1; i++) {
      if (
        boats[0].values[i].pos !== -1 &&
        boats[1].values[i].pos !== -1 &&
        (boats[0].values[i + 1].pos !== -1 && boats[1].values[i + 1].pos !== -1)
      ) {
        corr +=
          (boats[0].values[i + 1].pos - boats[0].values[i].pos) *
          (boats[1].values[i + 1].pos - boats[1].values[i].pos);
        len += 1;
      }
    }
    ...
    return (
      <div style={{ marginLeft: '20px' }}>
        ...
        <table>
          ...
          <tr>
            <td>
              {menHeadship.club} ({menHeadship.days})
            </td>
            <td>
              {womenHeadship.club} ({womenHeadship.days})
            </td>
            <td>
              Current Head of the River and number of days rowing over head
            </td>
          </tr>
          <tr>
            <td>Oriel (32)</td>
